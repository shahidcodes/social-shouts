...
class User{
	...
	public function loginByGoogleID($id)
	{
		if ($id) {
			// find and store user data and call ->login
			$this->findByGoogleID($id);
			return true;
		}
		return false;
	}
	...
	{
		if ($id) {
			// find user and return new user object
			$userData = $this->_db->get("users", ["google_id", "=", $id]);
			// user exists
			if ($userData->count()) {
				$this->_data = $userData->first();
				$this->login();
				return $this; // return object to chain login method
			}
		}
		return false;
	}
	...
	public function logout()
	{
		...
		Session::delete(Config::get("session/token_name")); // delete google auth
